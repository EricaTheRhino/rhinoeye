...
public class FrameDeltaServo{
	...
	public static void main(String[] args) throws Exception {
		...
		for (;e.hasNextFrame();) {
			...
			if(prior.x - middle.x < 0 ){
				cont.servoDelta(- cont.PWMdelta);
			}
			else{
				cont.servoDelta(+ cont.PWMdelta);
			}
			prevGrey = e.getNextFrame().flatten();


			System.out.println(String.format("Took: %dms, Average movement pos: %s, ",t.duration(),middle));
		}
	}
